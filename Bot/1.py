...
headers_vision = {'Ocp-Apim-Subscription-Key': '6dc1b01fda5d484db2e8b0d6eb0d0420'}
...
def calculate_move(gamestate):
    ...
    # If we don't have any matching tiles
        ...
        # If the unanalysed_tiles list is empty (all tiles have been analysed)
            ...
            move = move = sample(unanalysed_tiles, 2)
            if move in analysed_tiles:
                pass
...
def microsoft_api_call(url, params, headers, data):
    ...
    while ("error" in res and res["error"]["code"] == "429") or res == {}:
      ...
      try:
        response = requests.post(url, params=params, headers=headers, json=data, timeout=timeout_duration)
